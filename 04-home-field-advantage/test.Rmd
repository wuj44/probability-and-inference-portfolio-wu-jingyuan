...
the American and National Leagues of Major League Baseball. This blog is going to follow the previous one to continue to explain the probability calculation problems related to World Series with or without home filed advantage.
...
In any given game, the probability that the Braves wins is *P*<sub>*B*</sub> and the probability that the Yankees wins is *P*<sub>*Y*</sub> = 1 − *P*<sub>*B*</sub>.
...
First, list a table of all combinations that the Braves wins the World Series in 7 games. The first few rows of the table are shown below.
...
The probability that the Braves wins in any given game without home filed advantage is 0.55.

As the sequence of game locations is {NYC, NYC, ATL, ATL, ATL, NYC, NYC}, set **hfi <- c(0,0,1,1,1,0,0)**, 0 for NYC, 1 for ATL.

`advantage_multiplier` stands for the coefficient with home filed advantage, which is 1.1.
...
The chunk below is going to calculate the probability of each possible outcome in a `data.table` syntax. Double for loop is applied to iterate the probability that the Braves wins the game depending on the location. p stands for the probability that the Braves wins each game.
...
Based on the analytic solution, the probability with home field advantage when *P*<sub>*B*</sub> = 0.55 is 0.604221.
...
Based on the analytic solution, the probability without home field advantage when *P*<sub>*B*</sub> = 0.55 is 0.6082878.
...
The difference in probabilities between with and without home filed advantage is -0.004066825. Since the Braves's home is ATL and the sequence of game locations is {NYC, NYC, ATL, ATL, ATL, NYC, NYC}, it is reasonable that the probability with home filed advantage is slightly less than the probability without home filed advantage.
...
`times` is the number of simulations, which indicates that this simulation will iterates 100000 times.
...
Based on the simulation solution, the probability with home field advantage when *P*<sub>*B*</sub> = 0.55 is 0.60439.
...
Based on the simulation solution, the probability without home field advantage when *P*<sub>*B*</sub> = 0.55 is 0.60846.
...
In the simulation with home filed advantage, the absolute error is 0.000169028386345471, and the relative error is 0.000279745977525503.
...
In the simulation without home filed advantage, the absolute error is 0.00017220312499977, and the relative error is 0.000283094821044317.
...
Enumerate `pb` in the range of (0.5,1) with the break 0.01.

The sequence of game locations is {NYC, NYC, ATL, ATL, ATL, NYC, NYC}.

`advantage_multiplier` is 1.1.

Triple for loop respectively iterates `pb`, the probability each game that the Braves wins with home field advantage and 7 games.

`diff` is the difference between the probability with and without home field advantage.

A plot will be generated below to show the relationship between `Probability of the Braves Winning a Head-to-Head Matchup` and `Difference in Probabilities (with vs without Home Field Advantage)`.
...
The graph above shows that there is a trend of first rising and then falling. The difference in probabilities (with vs without home field advantage) does depend on *P*<sub>*B*</sub>. When *P*<sub>*B*</sub> = 0.71, the difference reaches maximum, which approximately equals to 0.01776, which indicates that the home filed advantage reaches most.
...
Enumerate `advantage_multiplier` in the range of (1,2) with the break 0.01.

Triple for loop respectively iterates `advantage_multiplier`, the probability each game that the Braves wins with home field advantage and 7 games.

`diff` is the difference between the probability with and without home field advantage.

A plot will be generated below to show the relationship between `Home Field Advantage Multiplier` and `Difference in Probabilities (with vs without Home Field Advantage)`.
...
The graph above shows a decreasing trend. The difference in probabilities decreases as home field advantage multiplier increases. As the Yankees has 4 home fields while the Braves has only 3, when home field advantage multiplier increases, the Braves has less home filed advantage over overall outcomes.
