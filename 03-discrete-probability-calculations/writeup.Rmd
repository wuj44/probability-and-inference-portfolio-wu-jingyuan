...
The calculation formula remains unchanged. `ceiling()` function is used for returning the smallest integer that is greater than or equal to the value passed to it as argument, which is in order to find the `losses at most` of winning a best-of-X series. A for loop is set for X to find the shortest series length. While P(Braves win World Series|PB = .55) ≥ 0.8, the for loop breaks and outputs the the shortest series length.
...
As there are two PB listed in Q5, it is easy to find that: P(A) = P(C) = 0.5
...
Based on the `Bayes Rules`, $$P(A|B) = P(B|A)P(A)/P(B)$$

P(A|B) = dnbinom(3,4,.55)*0.5/{[dnbinom(3,4,.55)+dnbinom(3,4,.45)]*0.5}
