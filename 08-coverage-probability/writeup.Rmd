...
boot_ci <- function(data, summary = "median"){
  ...
  sampdist <- rep(NA, R)
  for(i in 1:R){
    b <- rnorm(201, mean = data[2], sd = data[3])
    sampdist[i] <- sm(b)
  }
...
Generate a single sample from a standard normal distribution of size N=201. Take a look at the output, see how these functions work on a single sample.
